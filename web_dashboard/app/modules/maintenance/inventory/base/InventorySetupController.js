app.controller('InventorySetupController', ["$scope", "$rootScope", "$timeout", "$state", "$q", "GenericFactory", function ($scope, $rootScope, $timeout, $state, $q, GenericFactory) {
	...
	$scope.inventory.cstatus = "A";
	$scope.inventory.ccost = "A";
	...
	$scope.loadItemsList = function () {
		$scope.items_list = [];
		GenericFactory.get("icitem", "", custom_headers).then(function (response) {
			if (response.status) {
